...
-- AI 智能投标系统 · 数据库初始迁移脚本
-- Story: S0-008
-- 覆盖：全部 6 个 Schema（auth / project / knowledge / bid / ai_task / audit）
-- 包含：表、索引、触发器、视图、物化视图、分区、函数、种子数据
-- 执行顺序：Schema → 扩展/函数/枚举 → 表 → 索引 → 分区 → 视图 → 触发器
-- 注意：本文件由 docs/database-design.md 自动生成，请勿手动修改
-- ============================================================
...
CREATE TABLE bid.section_versions (
    ...
    CONSTRAINT uq_section_versions UNIQUE (section_id, version_no, created_at),
...
CREATE INDEX idx_qualifications_expiring    ON knowledge.qualifications (expire_date)
    WHERE deleted_at IS NULL AND is_expired = FALSE AND expire_date IS NOT NULL;
